...
const collectionAddy = '0x3bf2922f4520a8ba0c2efc3d2a1539678dad5e9d';
const tokenTransfersCollection = `https://api.etherscan.io/api?module=account&action=tokennfttx&contractaddress=0x3bf2922f4520a8ba0c2efc3d2a1539678dad5e9d&page=1&offset=100&startblock=0&endblock=99999999&sort=desc&apikey=${key}`;


async function OnForce (currentEthereumPrice) {
    
    let sortedArray = [];
    let recentTransactionBlock = '';

    await Collection.findOne({ nftName: "ON1"})
    ...
    const findNftTransactions = async () => {
        const transfers = await axios.get(`https://api.etherscan.io/api?module=account&action=tokennfttx&contractaddress=0x3bf2922f4520a8ba0c2efc3d2a1539678dad5e9d&page=1&offset=10000&startblock=${recentTransactionBlock}&endblock=99999999&sort=desc&apikey=${key}`);
    ...
    await Collection.findOneAndUpdate({ nftName: "ON1"}, {
